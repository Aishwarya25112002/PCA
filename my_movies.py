# -*- coding: utf-8 -*-
"""
Created on Tue Nov 21 16:53:06 2023

@author: hp
        DataSet-My_Movies
Business Objective:
Maximize: Audience engagement,profit
Minimize: Production time and production cost
Contraint: The business may face constraints
           related to market competition
"""
# DATA DICTIONARY-
"""
Name of 
Feature                Description                                           DataTypes                 Relevevance

(All Movies)        ( Type of movie)
Sixth Sense        Thriller, Drama, Mystery                                   Nominal                      Relevant
Gladiator          Historical Drama/Action                                    Nominal                      Relevant
LOTR1              Fantasy                                                    Nominal                      Relevant
Harry Potter1      Fantasy, Adventure                                         Nominal                       Relevant
Patriot            Action, Drama, Historical                                  Nominal                       Relevant
LOTR2              The Lord of the Rings: The Two Towers                      Nominal                       Relevant
                   Fantasy, Adventure
Harry Potter2      Fantasy                                                     Nominal                       Relevant
LOTR               (Lord of the Rings)
                   which is a high-fantasy novel                               Nominal                        Relevant

Braveheart         Historical Drama, Epic                                      Nominal                        Relevant
Green Mile         The Green Mile - drama, fantasy, and crime.                 Nominal                        Relevant


#All the columns is of nominal there is no ordinal column in the dataset
"""

#import the Library
import pandas as pd 
import numpy as np
import matplotlib.pyplot as plt
import seaborn as sns

#Load the dataset 

df=pd.read_csv("C:/8-PCA/my_movies.csv")
df

###########################
#Explore the data
# Display basic information about the dataset
print(df.info())
'''
Data columns (total 10 columns):
 #   Column         Non-Null Count  Dtype
---  ------         --------------  -----
 0   Sixth Sense    10 non-null     int64
 1   Gladiator      10 non-null     int64
 2   LOTR1          10 non-null     int64
 3   Harry Potter1  10 non-null     int64
 4   Patriot        10 non-null     int64
 5   LOTR2          10 non-null     int64
 6   Harry Potter2  10 non-null     int64
 7   LOTR           10 non-null     int64
 8   Braveheart     10 non-null     int64
 9   Green Mile     10 non-null     int64
dtypes: int64(10)
memory usage: 928.0 bytes
'''
# Display summary statistics
print(df.describe())

# Display the first few rows of the dataset
print(df.head())
#it can be display the first 5 rows
#####################
df.columns
'''
['Sixth Sense', 'Gladiator', 'LOTR1', 'Harry Potter1', 'Patriot',
       'LOTR2', 'Harry Potter2', 'LOTR', 'Braveheart', 'Green Mile']
'''
#Display how many column in given dataset

###################################
df.shape
#(303,14)
#Their are 303 rows and 14 columns in the dataset

###################################
df.dtypes
'''
Sixth Sense      int64
Gladiator        int64
LOTR1            int64
Harry Potter1    int64
Patriot          int64
LOTR2            int64
Harry Potter2    int64
LOTR             int64
Braveheart       int64
Green Mile       int64
dtype: object
'''
# All the data in the dataset is of numeric type
####################################
df.info()
'''
#   Column         Non-Null Count  Dtype
---  ------         --------------  -----
 0   Sixth Sense    10 non-null     int64
 1   Gladiator      10 non-null     int64
 2   LOTR1          10 non-null     int64
 3   Harry Potter1  10 non-null     int64
 4   Patriot        10 non-null     int64
 5   LOTR2          10 non-null     int64
 6   Harry Potter2  10 non-null     int64
 7   LOTR           10 non-null     int64
 8   Braveheart     10 non-null     int64
 9   Green Mile     10 non-null     int64
dtypes: int64(10)
'''
# It will show all the information about the columns
#####################################
a=df.describe()
a
# The mean and median are nearly same but shows some variation in the data
# the standard deviation is not equal to zero means the datapoints is in scatter form
#################################
#        Data Preprocessing
#Data cleaning

# Handling missing values
df.dropna(inplace=True)

# Removing duplicates
df.drop_duplicates(inplace=True)

# Handling outliers 
from scipy import stats
df = df[(np.abs(stats.zscore(df)) < 3).all(axis=1)]
df

# Check for the null values
v=df.isnull()
v.sum()
'''
Sixth Sense      0
Gladiator        0
LOTR1            0
Harry Potter1    0
Patriot          0
LOTR2            0
Harry Potter2    0
LOTR             0
Braveheart       0
Green Mile       0
dtype: int64
'''
# There is no null value in the dataset
##################################

# Visualization of data

# Plot the boxplot for outlier analysis
sns.boxplot(df,x='Sixth Sense')
# No Outlier
sns.boxplot(df,x='Gladiator')
# No Outlier
sns.boxplot(df,x='LOTR1')
# having one outlier
sns.boxplot(df)
# There is some outlier in the data

# Plot  the pairplot to understand behaviour
sns.pairplot(df)
# The datapoints are in scatter form

corr=df.corr()
sns.heatmap(corr)
# The diagonal colour of the heatmap is same so it follow some patern to understand
# the pattern we will perform some oerations
########################################
#           Normalization
#The data is numeric one so we have to perform normalization

def norm_fun(i):
    x=(i-i.min())/(i.max()-i.min())
    return x

df_norm=norm_fun(a)
df_norm

b=df_norm.describe()

sns.boxplot(df_norm)
# No Outlier is remaining
# The all the quantile points are converted in the rande of 0-1

############# Model Building ################
# Association Rules
from mlxtend.frequent_patterns import apriori,association_rules

data=pd.read_csv('my_movies.csv')
data

# All the data is in properly separated form so no need to apply the encoding techique
# as it is already is in the form of numeric one

from collections import Counter
item_frequencies=Counter(data)

# Apriori algorithm
frequent_itemsets = apriori(data, min_support=0.05, use_colnames=True)

# Generate association rules
rules = association_rules(frequent_itemsets, metric="confidence", min_threshold=0.5)

# This generate association rule for columns
# comprises of antescends,consequences

rules.head(20)
rules.sort_values('lift',ascending=False).head(10)

# Visualize the rules
import networkx as nx
import matplotlib.pyplot as plt

# Create directed graph from the rules
G = nx.from_pandas_edgelist(rules, 'antecedents', 'consequents')

# Draw the graph
fig, ax = plt.subplots(figsize=(14, 8))
pos = nx.spring_layout(G)
nx.draw(G, pos, with_labels=True, node_size=2500, node_color="skyblue", font_size=10, font_color="black", font_weight="bold", edge_color="gray", linewidths=1, alpha=0.7)
plt.title("Association Rules Network", fontsize=15)
plt.show()
##############################
#      Clustering-

import pandas as pd
from sklearn.cluster import KMeans
from sklearn.preprocessing import StandardScaler
import matplotlib.pyplot as plt

# Load the dataset
df = pd.read_csv('my_movies.csv')

# Assuming you have selected relevant features for clustering, e.g., 'feature1' and 'feature2'
selected_features = ['feature1', 'feature2']
X = df[selected_features]

# Standardize the data
scaler = StandardScaler()
X_scaled = scaler.fit_transform(X)

# Choose the number of clusters (you may need to adjust this based on your data)
num_clusters = 3

# Apply k-means clustering
kmeans = KMeans(n_clusters=num_clusters, random_state=42)
df['cluster'] = kmeans.fit_predict(X_scaled)

# Visualize the clusters (assuming 2D features)
plt.scatter(X_scaled[:, 0], X_scaled[:, 1], c=df['cluster'], cmap='viridis', edgecolor='k')
plt.title('K-Means Clustering of My Movies Data')
plt.xlabel('Feature 1 (Standardized)')
plt.ylabel('Feature 2 (Standardized)')
plt.show()


###################################################

# By using the association rule we will suggest the movies for the customer to 
# increase the viewers of the movies 
# Also by using this rules the revnue and the popularity of the movies
# will increases







